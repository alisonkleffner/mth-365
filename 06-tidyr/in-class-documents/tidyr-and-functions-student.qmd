...
```{r}
ff_long <- pivot_longer(french_fries, cols = potato:painty,
                        names_to = "variable",
                        values_to = "value")
...
```{r}
ggplot(ff_long) + geom_boxplot(aes(x=variable, y = value, fill = variable))
```
...
(df$a - min(df$a, na.rm = TRUE)) /
  (max(df$a, na.rm = TRUE) - min(df$a, na.rm = TRUE)) #one input: df$a
```
...
calculate_discount(40)
calculate_discount(60)
```
